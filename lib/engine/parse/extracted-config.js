...
class ExtractedConfig {
  ...
  handleBuild(services) {
    Object.keys(services).forEach((key) => {
      ...
      cmdList.filter(el => !['build', 'push'].includes(el)).forEach((cmd) => {
        this.builds[key][cmd] = service[cmd] || []
      })
  ...
  handleMatrices(matrices) {
    ...
    _matrices.forEach((_matrix) => {
      ...
      (_matrix.build_arguments || _matrix.buildArguments || [])
        .forEach((argument) => argumentMap.set(...argument.split('=')))
      matrix['buildArguments'] = argumentMap
