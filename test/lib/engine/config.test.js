...
describe('ConfigFactory', function () {
  describe('loadFile', function () {
    ...
    it("should throw an error if 'filePath' was 'package.json' and it doesn't have 'advtrConfig' field.", () => {
      assert.throws(() => factory.loadFile('package-json-no-config/package.json'))
    })
  ...
  describe('loadInDirectory', function () {
    ...
    fs.defineTempFiles({
      ...
      files: {
        ...files,
        'package-json-no-config/package.json': '{ "name": "foo" }',
        'invalid-json/.advtrc.json': '{hello: world}'
      }
    })
    ...
    it('should throw an error for malformed json', () => {
      assert.throws(() => factory.loadInDirectory('invalid-json'))
    })
